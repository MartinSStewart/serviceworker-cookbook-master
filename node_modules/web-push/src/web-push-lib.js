...
WebPushLib.prototype.generateRequestDetails = function(subscription, payload, options) {
    ...
    function jwsSecuredInput(header, payload) {
      var encodedHeader = base64url(JSON.stringify(header), 'binary');
      var encodedPayload = base64url(JSON.stringify(payload), 'utf8');
...
//    });
    ...
    const securedInput = jwsSecuredInput(header, jwtPayload);
    const signature = algo.sign(securedInput, pemKey);
    const jwt = util.format('%s.%s', securedInput, signature);
