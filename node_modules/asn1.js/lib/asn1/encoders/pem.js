...
DERNode.prototype._encodeStr = function encodeStr(str, tag) {
  if (tag === 'bitstr') {
    return new EncoderBuffer([ str.unused | 0, str.data ]);
...
DERNode.prototype._encodeObjid = function encodeObjid(id) {
  ...
  return new EncoderBuffer(objid);
};
...
DERNode.prototype._encodeInt = function encodeInt(num, values) {
  ...
  if (Buffer.isBuffer(num)) {
    ...
    if (num.length === 0)
      out[0] = 0;
    return new EncoderBuffer(out);
  }

  if (num < 0x80)
    return new EncoderBuffer(num);

  if (num < 0x100)
    return new EncoderBuffer([0, num]);
  ...
  return new EncoderBuffer(Buffer.from(out));
};
